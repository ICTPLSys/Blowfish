...
#define AUTO_RECLAIM_CPU_STATS
#ifdef AUTO_RECLAIM_CPU_STATS
static uint64_t auto_reclaim_start_time; 
static uint64_t auto_reclaim_total_cpu_time;
static unsigned long auto_reclaim_run_count;
static bool auto_reclaim_first_run = true;
#endif

#define SOFT_RECLAIM_STATS
#ifdef SOFT_RECLAIM_STATS
static uint64_t soft_reclaim_total_time;
static unsigned long soft_reclaim_call_count;
static uint64_t soft_reclaim_max_time;
static uint64_t soft_reclaim_min_time = UINT64_MAX;

static uint64_t notify_discard_total_time;
static unsigned long notify_discard_call_count;
static uint64_t notify_discard_max_time;
static uint64_t notify_discard_min_time = UINT64_MAX;

/* notify_populate timing */
static uint64_t notify_populate_total_time;
static unsigned long notify_populate_call_count;
static uint64_t notify_populate_max_time;
static uint64_t notify_populate_min_time = UINT64_MAX;

/* handle_install_request timing */
static uint64_t install_request_total_time;
static unsigned long install_request_call_count;
static uint64_t install_request_max_time;
static uint64_t install_request_min_time = UINT64_MAX;
#endif
// For Testing
...
#define LL_RETRY_INTERVAL (0.1 * NANOSECONDS_PER_SECOND)
#define LL_PAGECACHE_RETRIES 10
...
static void unmap_range(VirtIOLLFreeBalloon *dev, hwaddr gpa, size_t size)
{
    ...
    notify_discard_total_time += exec_time;
    notify_discard_call_count++;

    if (exec_time > notify_discard_max_time) {
        notify_discard_max_time = exec_time;
    }
    if (exec_time < notify_discard_min_time) {
        notify_discard_min_time = exec_time;
    }
#endif
...
static int soft_reclaim_cb(void *ctx, uint64_t frame, size_t len)
{
    ...
    soft_reclaim_total_time += exec_time;
    soft_reclaim_call_count++;
    
    if (exec_time > soft_reclaim_max_time) {
        soft_reclaim_max_time = exec_time;
    }
    if (exec_time < soft_reclaim_min_time) {
        soft_reclaim_min_time = exec_time;
    }
#endif
    return 0;
}
...
static int32_t ll_auto_soft_reclaim(VirtIOLLFreeBalloon *dev)
{
    ...
    auto_reclaim_total_cpu_time += cpu_time;
    auto_reclaim_run_count++;
#endif
...
static void map_range(VirtIOLLFreeBalloon *dev, hwaddr gpa, uint64_t size)
{
    ...
    if (virtio_has_feature(dev->host_features, LL_BALLOON_F_VFIO)) {
        ...
        notify_populate_total_time += exec_time;
        notify_populate_call_count++;
        
        if (exec_time > notify_populate_max_time) {
            notify_populate_max_time = exec_time;
        }
        if (exec_time < notify_populate_min_time) {
            notify_populate_min_time = exec_time;
        }
#endif
    }

    int ret = qemu_madvise(host_addr, size, 23);
    if (ret != 0)
...
-------------------------------------------------------------------------------------------------*/
// #define INCONTINUOUS_TEST
static bool ll_try_shrink_pagecache(VirtIOLLFreeBalloon *dev,
                                    uint32_t diff_huge_pages)
{
    VirtIODevice *vdev = VIRTIO_DEVICE(dev);

    // extend to reclaim all pages
    if(dev->shrink_pagecache == 0) {
        size_t target = diff_huge_pages * LL_HUGE_COUNT;
        if(dev->pagecache_retries > 0) {
#ifndef INCONTINUOUS_TEST
            target /= 2;
#else
            return true;
#endif
                                   ...
                                   bool shrink_pagecache)
{
    ...
    if (diff_huge_pages > 0 && 
        ...
        // if (shrink_pagecache) {
            ll_try_shrink_pagecache(dev,  diff_huge_pages);
        // }
        // Retry inflation later
        timer_mod(dev->retry_llfree_timer,
                    qemu_clock_get_ns(QEMU_CLOCK_HOST) + 0.1 * NANOSECONDS_PER_SECOND);
    }
    return;
#ifdef INCONTINUOUS_TEST
    hard_reclaim_node(dev, 0, 0);
#endif
}
...
static void ll_balloon_auto_reclaim(void *opaque)
{
    VirtIOLLFreeBalloon *dev = opaque;
#ifdef AUTO_RECLAIM_CPU_STATS
    if (auto_reclaim_first_run) {
        auto_reclaim_start_time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
        auto_reclaim_total_cpu_time = 0;
        auto_reclaim_run_count = 0;
        auto_reclaim_first_run = false;
    }
#endif
...
static void handle_install_request(VirtIODevice *vdev, VirtQueue *vq)
{
    ...
    install_request_total_time += exec_time;
    install_request_call_count++;
    
    if (exec_time > install_request_max_time) {
        install_request_max_time = exec_time;
    }
    if (exec_time < install_request_min_time) {
        install_request_min_time = exec_time;
    }
#endif
...
static void ll_balloon_stat(void *opaque, LLfreeBalloonInfo *info)
{
    ...
    if (!auto_reclaim_first_run) {
        ...
        uint64_t elapsed_time = current_time - auto_reclaim_start_time;
        double cpu_usage = 0;
        
        if (elapsed_time > 0) {
            cpu_usage = ((double)auto_reclaim_total_cpu_time / elapsed_time) * 100.0;
        }
        
        info_report("Current auto reclaim CPU usage: %.2f%% (runs: %lu, total_cpu_time: %lu ns, elapsed_time: %lu ns, average_run_time: %lu ns)",
                   cpu_usage, auto_reclaim_run_count,
                   auto_reclaim_total_cpu_time, elapsed_time,
                   auto_reclaim_run_count > 0 ? auto_reclaim_total_cpu_time / auto_reclaim_run_count : 0);
    }
#endif
#ifdef SOFT_RECLAIM_STATS
    if (soft_reclaim_call_count > 0) {
        uint64_t avg_time = soft_reclaim_total_time / soft_reclaim_call_count;
        
        info_report("Soft reclaim statistics: calls: %lu, avg_time: %lu ns, min_time: %lu ns, max_time: %lu ns, total_time: %lu ns",
                   soft_reclaim_call_count, avg_time,
                   soft_reclaim_min_time == UINT64_MAX ? 0 : soft_reclaim_min_time,
                   soft_reclaim_max_time, soft_reclaim_total_time);
    }
    if (notify_discard_call_count > 0) {
        uint64_t avg_time = notify_discard_total_time / notify_discard_call_count;
        
        info_report("Notify discard statistics: calls: %lu, avg_time: %lu ns, min_time: %lu ns, max_time: %lu ns, total_time: %lu ns",
                   notify_discard_call_count, avg_time,
                   notify_discard_min_time == UINT64_MAX ? 0 : notify_discard_min_time,
                   notify_discard_max_time, notify_discard_total_time);
    }
    if (notify_populate_call_count > 0) {
        uint64_t avg_time = notify_populate_total_time / notify_populate_call_count;
        
        info_report("Notify populate statistics: calls: %lu, avg_time: %lu ns, min_time: %lu ns, max_time: %lu ns, total_time: %lu ns",
                   notify_populate_call_count, avg_time,
                   notify_populate_min_time == UINT64_MAX ? 0 : notify_populate_min_time,
                   notify_populate_max_time, notify_populate_total_time);
    }
    if (install_request_call_count > 0) {
        uint64_t avg_time = install_request_total_time / install_request_call_count;
        
        info_report("Install request statistics: calls: %lu, avg_time: %lu ns, min_time: %lu ns, max_time: %lu ns, total_time: %lu ns",
                   install_request_call_count, avg_time,
                   install_request_min_time == UINT64_MAX ? 0 : install_request_min_time,
                   install_request_max_time, install_request_total_time);
    }
#endif
...
#ifdef AUTO_RECLAIM_CPU_STATS
    auto_reclaim_first_run = true;
#endif
#ifdef SOFT_RECLAIM_STATS
    soft_reclaim_total_time = 0;
    soft_reclaim_call_count = 0;
    soft_reclaim_max_time = 0;
    soft_reclaim_min_time = UINT64_MAX;
    notify_discard_total_time = 0;
    notify_discard_call_count = 0;
    notify_discard_max_time = 0;
    notify_discard_min_time = UINT64_MAX;
    notify_populate_total_time = 0;
    notify_populate_call_count = 0;
    notify_populate_max_time = 0;
    notify_populate_min_time = UINT64_MAX;
    install_request_total_time = 0;
    install_request_call_count = 0;
    install_request_max_time = 0;
    install_request_min_time = UINT64_MAX;
#endif
