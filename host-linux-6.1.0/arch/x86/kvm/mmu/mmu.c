...
/* fault->gfn: pending restore set at KVM_HC_BLOWFISH_PAGEOUT_RECLAIM / async reclaim. */
static void kvm_blowfish_fault_restore(struct kvm *kvm, struct kvm_page_fault *fault)
{
	u64 *buf;
	unsigned long n, idx;

	if (!kvm_blowfish_consume_pending_gfn(kvm, fault->gfn))
		return;

	buf = READ_ONCE(kvm->blowfish_restore_buffer);
	n = READ_ONCE(kvm->blowfish_restore_buffer_size);
	if (!buf || !n)
		return;

	idx = (unsigned long)(atomic_inc_return(&kvm->blowfish_restore_buffer_idx) - 1) % n;
	while (READ_ONCE(buf[idx])) {
		pr_info("kvm_blowfish_fault_restore: idx=%lu, gfn=%llx\n", idx, fault->gfn);
		if (fatal_signal_pending(current)) {
			pr_info("kvm_blowfish_fault_restore: fatal_signal_pending\n");
			return;
		}
		if (READ_ONCE(kvm->blowfish_restore_buffer) != buf ||
		    READ_ONCE(kvm->blowfish_restore_buffer_size) != n)
			return;
		schedule();
	}
	WRITE_ONCE(buf[idx], fault->gfn);
}
...
out_unlock:
	...
	if (r == RET_PF_FIXED || r == RET_PF_SPURIOUS)
		kvm_blowfish_fault_restore(vcpu->kvm, fault);
	// kvm_vfio_check_pfn_consistency(vcpu, fault, r);
