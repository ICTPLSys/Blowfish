...
static unsigned long long int dma_entry_limit __read_mostly = U64_MAX;
module_param_named(dma_entry_limit, dma_entry_limit, ullong, 0644);
...
struct vfio_dma {
	...
	unsigned long		pfn;		/* First pinned host pfn for profiling */
...
static long vfio_pin_pages_remote_profiling(struct vfio_dma *dma, unsigned long vaddr,
				  long npage, unsigned long *pfn_base,
				  unsigned long limit, struct vfio_batch *batch)
{
	unsigned long pfn;
	// the different
	struct mm_struct *mm = dma->task->mm;
	long ret, pinned = 0, lock_acct = 0;
	bool rsvd;
	dma_addr_t iova = vaddr - dma->vaddr + dma->iova;
...
#define FINEGRAINED_DMA_SUPPORT

#ifdef FINEGRAINED_DMA_SUPPORT

static int vfio_dma_do_map_use_once(struct vfio_iommu *iommu,
	struct vfio_iommu_type1_dma_map *map, struct task_struct *task);
static int vfio_dma_do_map_once(struct vfio_iommu *iommu,
		struct vfio_iommu_type1_dma_map *map);

int handle_unmap(struct kvm* kvm, uint64_t iova, uint64_t size)
{
	struct vfio_iommu *iommu = (struct vfio_iommu *)kvm->coordinator_iommu;
	u64 cur_iova = iova, left = size;
	int ret = 0;

	if (!kvm->reporting_buffers_registered)
		return 0;

	if (!iommu || (iova | size) & (PAGE_SIZE - 1))
		return -EINVAL;
	...
	while (left) {
		...
		if (ret) {
			pr_info("handle_unmap: ret: %d, iova: %llx, size: %llx\n", ret,
				(unsigned long long)unmap.iova,
...
int handle_map(struct kvm* kvm, uint64_t iova, uint64_t size)
{
	...
	if (!iommu || !kvm->mm || !kvm->mm->owner || (iova | size) & (PAGE_SIZE - 1))
		return -EINVAL;
	...
	while (left) {
		struct vfio_iommu_type1_dma_map map = {
			...
			.vaddr = gfn_to_hva(kvm, (u64)cur_iova >> PAGE_SHIFT),
			.iova = cur_iova,
			.size = PAGE_SIZE,
		};

		// ret = vfio_dma_do_map_use_once(iommu, &map, kvm->mm->owner);
		ret = vfio_dma_do_map_once(iommu, &map);
		
		// if (ret) {
		// 	pr_info("handle_map: ret: %d, iova: %llx, vaddr: %llx\n", ret, (unsigned long long)map.iova, (unsigned long long)map.vaddr);
		// 	// return ret;
		// }
...
static int __handle_get_pfn_nolock(struct vfio_iommu *iommu, uint64_t iova,
				   unsigned long *pfn)
{
	struct vfio_dma *dma;
	struct vfio_domain *d;
	...
	dma_pfn = dma->pfn + ((iova - dma->iova) >> PAGE_SHIFT);
	*pfn = dma_pfn;
	...
	if (!list_empty(&iommu->domain_list)) {
		list_for_each_entry(d, &iommu->domain_list, next) {
			...
			if (!phys)
				continue;
			if ((phys >> PAGE_SHIFT) != dma_pfn) {
				pr_err("handle_get_pfn mismatch: iova=%llx dma_pfn=%lx iommu_pfn=%llx\n",
...
int handle_get_pfn(struct kvm *kvm, uint64_t iova, unsigned long *pfn)
{
	...
	if (!iommu || !pfn || (iova & (PAGE_SIZE - 1)))
		return -EINVAL;

	mutex_lock(&iommu->lock);
	ret = __handle_get_pfn_nolock(iommu, iova, pfn);
	mutex_unlock(&iommu->lock);
	return ret;
}

/**
 * handle_get_pfn_try - same as handle_get_pfn but never blocks on iommu->lock.
 * Returns -EBUSY when the lock is contended (caller must not sleep waiting).
 */
...
static int vfio_iommu_type1_link_iommu_kvm(struct vfio_iommu *iommu,
				    unsigned long arg) 
{
	struct fd f;
	struct kvm *kvm;
	...
	if (!f.file)
		return -EBADF;
	kvm = f.file->private_data;
	// mutex_lock(&kvm->lock);
	if(!kvm->coordinator_iommu)
		kvm->coordinator_iommu = (void*)iommu;
	if(!(kvm->vfio_iommu_func)) {
		kvm->vfio_iommu_func = kmalloc(sizeof(struct vfio_iommu_func), GFP_KERNEL);
		((struct vfio_iommu_func*)kvm->vfio_iommu_func)->handle_map = handle_map;
		((struct vfio_iommu_func*)kvm->vfio_iommu_func)->handle_unmap = handle_unmap;
		((struct vfio_iommu_func*)kvm->vfio_iommu_func)->handle_get_pfn = handle_get_pfn;
		((struct vfio_iommu_func*)kvm->vfio_iommu_func)->handle_get_pfn_try =
			handle_get_pfn_try;
	}
	// mutex_unlock(&kvm->lock);
	fdput(f);
	// printk("vfio_iommu_type1_link_iommu_kvm: kvm->coordinator_iommu: %p\n", kvm->coordinator_iommu);
	
	return 0;
}
static int vfio_iommu_type1_unlink_iommu_kvm(struct vfio_iommu *iommu,
				    unsigned long arg) 
{
	struct fd f;
	struct kvm *kvm;
	...
	kvm->coordinator_iommu = NULL;
	if(kvm->vfio_iommu_func)
		kfree(kvm->vfio_iommu_func);
	kvm->vfio_iommu_func = NULL;

	fdput(f);
	// printk("vfio_iommu_type1_unlink_iommu_kvm: kvm->coordinator_iommu: %p\n", kvm->coordinator_iommu);
	return 0;
}
#endif
...
static int vfio_pin_map_dma_profiling(struct vfio_iommu *iommu, struct vfio_dma *dma,
			    size_t map_size)
{
	dma_addr_t iova = dma->iova;
	unsigned long vaddr = dma->vaddr;
	...
	while (size) {
		/* Pin a contiguous chunk of memory */
		npage = vfio_pin_pages_remote_profiling(dma, vaddr + dma->size,
					      size >> PAGE_SHIFT, &pfn, limit,
					      &batch);
		// printk("vfio_pin_pages_remote: npage: %ld, pfn: %lx\n", npage, pfn);
		if (npage <= 0) {
			WARN_ON(!npage);
			ret = (int)npage;
			break;
		...
		// printk("vfio_iommu_map: iova: %llx, pfn: %lx, npage: %ld\n",
		//        (unsigned long long)iova + dma->size, pfn, npage);
...
#ifdef FINEGRAINED_DMA_SUPPORT
	bool set_vaddr = map->flags & VFIO_DMA_MAP_FLAG_VADDR;
	u64 left = map->size, cur_iova = map->iova, cur_vaddr = map->vaddr;
	int ret = 0;

	if (set_vaddr)
		return vfio_dma_do_map_once(iommu, map);

	if ((map->iova | map->vaddr | map->size) & (PAGE_SIZE - 1))
		return -EINVAL;

	while (left) {
		struct vfio_iommu_type1_dma_map split = *map;

		split.iova = cur_iova;
		split.vaddr = cur_vaddr;
		split.size = PAGE_SIZE;
		split.flags = map->flags;

		ret = vfio_dma_do_map_once(iommu, &split);
		if (ret) {
			pr_err("vfio_dma_do_map: vfio_dma_do_map_once failed ret=%d iova=%llx size=%llx\n", ret, (unsigned long long)cur_iova, (unsigned long long)left);
			return ret;
		}
				    ...
				    struct task_struct *task)
{
	...
	if (list_empty(&iommu->domain_list))
		...
		ret = vfio_pin_map_dma_profiling(iommu, dma, size);
	}
...
static int vfio_iommu_type1_map_dma_profiling(struct vfio_iommu *iommu,
				    unsigned long arg)
{
	struct vfio_iommu_type1_dma_map map;
	unsigned long minsz;
...
static int vfio_iommu_type1_unmap_dma_profiling(struct vfio_iommu *iommu,
				      unsigned long arg)
{
	struct vfio_iommu_type1_dma_unmap unmap;
				   ...
				   unsigned int cmd, unsigned long arg)
{
	...
	case VFIO_IOMMU_MAP_DMA_PROFILING:
		return vfio_iommu_type1_map_dma_profiling(iommu, arg);
	case VFIO_IOMMU_UNMAP_DMA_PROFILING:
		return vfio_iommu_type1_unmap_dma_profiling(iommu, arg);
	case VFIO_IOMMU_DIRTY_PAGES:
		return vfio_iommu_type1_dirty_pages(iommu, arg);
#ifdef FINEGRAINED_DMA_SUPPORT
	case VFIO_IOMMU_LINK_WITH_KVM:
		return vfio_iommu_type1_link_iommu_kvm(iommu, arg);
	case VFIO_IOMMU_UNLINK_WITH_KVM:
		return vfio_iommu_type1_unlink_iommu_kvm(iommu, arg);
#endif
	default:
		return -ENOTTY;
	}
