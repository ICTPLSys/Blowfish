...
int swap_writepage(struct page *page, struct writeback_control *wbc)
{
	...
	if (frontswap_store(&folio->page, offset, folio_size(folio)) == 0) {
		...
		count_vm_events(PGPGOUT, 4 * 2); // fake sector count for vmstat.c v[PGPGOUT] /= 2;
...
int swap_writepage_on_core(struct page *page, struct writeback_control *wbc, int core)
{
	...
	pf_ts = get_cycles_start();
	if (frontswap_store_on_core(&folio->page, core, folio_size(folio)) == 0) {
		// folio_start_writeback(folio);
		// folio_unlock(folio);
		// folio_end_writeback(folio);
		count_vm_event(PSWPOUT);
		  ...
		  struct swap_iocb **plug)
{
	...
	pf_ts = get_cycles_start();
	if (frontswap_load(page, offset, thp_size(page)) == 0) {
		// SetPageUptodate(page);
		// unlock_page(page);
		accum_adc_time_stat(ADC_RDMA_READ_LAT,
				    get_cycles_end() - pf_ts);
		count_vm_event(PSWPIN);
		count_vm_events(PGPGIN, 4 * 2); // fake sector count for vmstat.c v[PGPGIN] /= 2;
		goto out;
	}
		  ...
		  struct swap_iocb **plug)
{
	...
	delayacct_swapin_start();
	pf_ts = get_cycles_start();
	if (frontswap_load_async(page, offset, thp_size(page)) == 0) {
		// SetPageUptodate(page);
		// unlock_page(page);
		// accum_adc_time_stat(ADC_RDMA_READ_LAT,
		// 		    get_cycles_end() - pf_ts);
