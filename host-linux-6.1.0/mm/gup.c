		...
		int *locked)
{
...

	{
		...
		if (current->adc_pf_bits && current->adc_pf_breakdown) {
			adc_pf_breakdown_stt(current->adc_pf_breakdown, ADC_EPT_VIOLATION,
					     get_cycles_start());
			ret = handle_mm_fault_profiling(vma, address, fault_flags, NULL,
						current->adc_pf_bits,
						current->adc_pf_breakdown);
			adc_pf_breakdown_end(current->adc_pf_breakdown, ADC_EPT_VIOLATION, get_cycles_end());
			set_adc_pf_bits(current->adc_pf_bits, ADC_PF_FIXED_BIT);
		} else {
...
static long __get_user_pages_profiling(struct mm_struct *mm,
		unsigned long start, unsigned long nr_pages,
		unsigned int gup_flags, struct page **pages,
		struct vm_area_struct **vmas, int *locked)
{
...
static __always_inline long __get_user_pages_locked_profiling(struct mm_struct *mm,
						unsigned long start,
						unsigned long nr_pages,
						struct page **pages,
						struct vm_area_struct **vmas,
						...
						unsigned int flags)
{
	...
	lock_dropped = false;
	for (;;) {
		ret = __get_user_pages_profiling(mm, start, nr_pages, flags, pages,
				       vmas, locked);
		if (!locked)
			/* VM_FAULT_RETRY couldn't trigger, bypass */
			return ret;
...
retry:
		...
		*locked = 1;
		ret = __get_user_pages_profiling(mm, start, 1, flags | FOLL_TRIED,
				       pages, NULL, locked);
		if (!*locked) {
			/* Continue to retry until we succeeded */
			BUG_ON(ret != 0);
...
static long __get_user_pages_remote_profiling(struct mm_struct *mm,
				    unsigned long start, unsigned long nr_pages,
				    unsigned int gup_flags, struct page **pages,
				    struct vm_area_struct **vmas, int *locked)
{
	...
	return __get_user_pages_locked_profiling(mm, start, nr_pages, pages, vmas,
				       locked,
				       gup_flags | FOLL_TOUCH | FOLL_REMOTE);
}
...
long get_user_pages_remote_profiling(struct mm_struct *mm,
		unsigned long start, unsigned long nr_pages,
		unsigned int gup_flags, struct page **pages,
		struct vm_area_struct **vmas, int *locked)
{
	...
	return __get_user_pages_remote_profiling(mm, start, nr_pages, gup_flags,
				       pages, vmas, locked);
}
EXPORT_SYMBOL(get_user_pages_remote_profiling);
