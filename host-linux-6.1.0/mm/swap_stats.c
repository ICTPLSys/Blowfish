...
static const char *adc_time_stat_names[NUM_ADC_TIME_STAT_TYPE] = {
	...
	"Fill in host latency", "Fill HPT EPT latency", "Fill write page latency",
	"Leak all2 latency", "Leak read page latency", "Leak help function latency",
	"Leak get page latency", "Leak help others latency",
	"Hypercall read latency", "Hypercall write latency",
	"Front swap read latency", "Front swap write latency",
	"Fill all latency", "Leak all latency", "Fill IOPT latency",
	"Leak IOPT latency", "Fill HPT latency", "Leak HPT latency", "FILL TLB latency",
	"Populate write latency", "Dontneed latency", "Unmap IOPT latency",
	"Map IOPT latency"
};
...
void report_adc_pf_breakdown(uint64_t *buf)
{
	...
	for (type = 0; type < NUM_ADC_PF_TYPE; type++) {
		...
		for (i = 0; i < NUM_ADC_PF_BREAKDOWN_TYPE; i++) {
			...
			printk("%s: %s \t%lluns, #: %d\n", adc_pf_type_names[type],
			       adc_pf_breakdown_names[i], duration, cnt);
		}
...
{
	/* KVM EPT Faults */
	if (get_adc_pf_bits(adc_pf_bits, ADC_PF_EPT_BIT)) {
		if (get_adc_pf_bits(adc_pf_bits, ADC_PF_IO_BIT)) {
			record_spf_lat(pf_breakdown);
			accum_adc_pf_breakdown(pf_breakdown, ADC_EPT_SPF);
		}
		if (get_adc_pf_bits(adc_pf_bits, ADC_PF_FIXED_BIT)) {
			// EPT fixed fault, do not record
			record_spf_lat(pf_breakdown);
...
EXPORT_SYMBOL_GPL(parse_adc_pf_breakdown);
#else
