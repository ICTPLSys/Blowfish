...
struct kvm {
	...
	//[coordinator]
	void* coordinator_iommu;
	void* vfio_iommu_func;

    u64 *reporting_gfn_buffer;
    u32 *reporting_len_buffer;
    gpa_t reporting_gfn_buffer_gpa;
    gpa_t reporting_len_buffer_gpa;
    unsigned int reporting_buffer_capacity;
    bool reporting_buffers_registered;
	unsigned long inflate_end;
	unsigned long deflate_end;
	unsigned long inflate_range;
	unsigned long deflate_range;

	u64 *blowfish_restore_buffer;
	unsigned long blowfish_restore_buffer_size;
	atomic_t blowfish_restore_buffer_idx;
	/* GFNs pending blowfish restore notification (see KVM_HC_BLOWFISH_PAGEOUT_RECLAIM). */
	struct xarray blowfish_pending_gfns;
	u64 *blowfish_reclaim_gfn_buffer;
	u32 *blowfish_reclaim_len_buffer;
	unsigned int blowfish_reclaim_buffer_capacity;
	bool blowfish_reclaim_buffers_registered;

	/* Ring async reclaim for KVM_HC_BLOWFISH_ASYNC_RECLAIM_REGISTER (see coordinator.h). */
	struct task_struct *blowfish_async_task;
	u64 *blowfish_async_buffer;
	unsigned int blowfish_async_n_slots;
};
