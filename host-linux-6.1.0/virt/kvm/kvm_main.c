...
static struct kvm *kvm_create_vm(unsigned long type, const char *fdname)
{
	...
	xa_init(&kvm->blowfish_pending_gfns);
	...
	kvm->reporting_gfn_buffer = NULL;
    kvm->reporting_len_buffer = NULL;
	kvm->reporting_gfn_buffer_gpa = 0;
	kvm->reporting_len_buffer_gpa = 0;
	kvm->reporting_buffer_capacity = 0;
	kvm->reporting_buffers_registered = false;
	
	kvm->blowfish_restore_buffer = NULL;
	kvm->blowfish_restore_buffer_size = 0;
	atomic_set(&kvm->blowfish_restore_buffer_idx, 0);
	kvm->blowfish_reclaim_gfn_buffer = NULL;
	kvm->blowfish_reclaim_len_buffer = NULL;
	kvm->blowfish_reclaim_buffer_capacity = 0;
	kvm->blowfish_reclaim_buffers_registered = false;
...
out_err_no_srcu:
	xa_destroy(&kvm->blowfish_pending_gfns);
...
static void kvm_destroy_vm(struct kvm *kvm)
{
	...
	xa_destroy(&kvm->blowfish_pending_gfns);
	vunmap(kvm->blowfish_restore_buffer);
	kvm->blowfish_restore_buffer = NULL;
	kvm->blowfish_restore_buffer_size = 0;
	vunmap(kvm->blowfish_reclaim_gfn_buffer);
	vunmap(kvm->blowfish_reclaim_len_buffer);
	kvm->blowfish_reclaim_gfn_buffer = NULL;
	kvm->blowfish_reclaim_len_buffer = NULL;
	kvm->blowfish_reclaim_buffer_capacity = 0;
	kvm->blowfish_reclaim_buffers_registered = false;
	...
	kvm->blowfish_async_buffer = NULL;
	kvm->blowfish_async_n_slots = 0;
	vunmap(kvm->reporting_gfn_buffer);
	vunmap(kvm->reporting_len_buffer);
	kvm->reporting_gfn_buffer = NULL;
	kvm->reporting_len_buffer = NULL;
...
static int hva_to_pfn_slow_profiling(unsigned long addr, bool *async, bool write_fault,
				bool *writable, kvm_pfn_t *pfn)
{
	unsigned int flags = FOLL_HWPOISON;
	struct page *page;
...
static kvm_pfn_t hva_to_pfn_profiling(struct mm_struct *mm, unsigned long addr, bool atomic, bool *async,
			bool write_fault, bool *writable)
{
	struct vm_area_struct *vma;
	kvm_pfn_t pfn;
	...
	npages = hva_to_pfn_slow_profiling(addr, async, write_fault, writable, &pfn);
	if (npages == 1)
		return pfn;
...
kvm_pfn_t __gfn_to_pfn_memslot_profiling(struct mm_struct *mm, struct kvm_memory_slot *slot, gfn_t gfn,
			       bool atomic, bool *async, bool write_fault,
			       bool *writable, hva_t *hva)
{
	unsigned long addr = __gfn_to_hva_many(slot, gfn, NULL, write_fault);
	...
	return hva_to_pfn_profiling(mm, addr, atomic, async, write_fault,
			  writable);
}
EXPORT_SYMBOL_GPL(__gfn_to_pfn_memslot_profiling);
