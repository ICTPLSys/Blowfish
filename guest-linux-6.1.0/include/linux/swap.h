...
unsigned long hermit_workingset_refault(struct folio *folio, void *shadow);
void *hermit_workingset_eviction(struct folio *folio, struct lruvec *lruvec,
					 int memcgid);
...
extern unsigned long hermit_try_to_free_mem_cgroup_pages(struct mem_cgroup *memcg,
						  unsigned long nr_pages,
						  gfp_t gfp_mask,
						  unsigned int reclaim_options,
						struct task_struct *cthd,
						int *adc_pf_bits, uint64_t pf_breakdown[]);
static inline unsigned long try_to_free_mem_cgroup_pages(struct mem_cgroup *memcg,
							  unsigned long nr_pages,
							  gfp_t gfp_mask,
							  unsigned int reclaim_options)
{
	return hermit_try_to_free_mem_cgroup_pages(memcg, nr_pages,
						    gfp_mask,
						    reclaim_options, NULL, NULL, NULL);
}
...
void hermit_mem_cgroup_swapout(struct folio* folio, struct mem_cgroup *memcg,
				      unsigned nr_entries);
