...
void ll_request_install(struct zone *zone, uint64_t frame, size_t core)
{
	...
	*info = (ll_install_info_t){
    ...
    accum_adc_time_stat(ADC_GUEST_FILL_LAT, pf_ts);  /* profiling bucket */
#endif
}
...
// 	printk(KERN_INFO
// 	       "llfree_balloon: reclaimed %u / %u pages\n",
// 	       reclaimed_pages, shrink_pagecache);
...
static void shrink_pagecache_func(struct work_struct *work)
{
    ...
    while (reclaimed_pages < shrink_pagecache) {
        uint32_t remaining = shrink_pagecache - reclaimed_pages;
        uint32_t current_batch = max(min(shrink_pagecache, remaining) / 2, batch_size);
        
        for_each_node(node) {
            reclaimed_pages += 
                shrink_pagecache_for_reclaim(node, current_batch / num_online_nodes() + 1);
        }
