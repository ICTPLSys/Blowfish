			...
			unsigned long end_pfn, isolate_mode_t mode)
{
	...
	for (; low_pfn < end_pfn; low_pfn++) {
		...
		if (!PageLRU(page)) {
			/*
			 * Direct compaction: parked Blowfish folios are off LRU
			 * on purpose.  Restore them (host-visible + LRU) so the
			 * same PFN can be re-scanned and isolated like a normal
			 * page.  folio_add_lru() is batched; drain before re-scan.
			 */
			if (cc->direct_compaction && blowfish_restore_reported_folio(
				    folio_pfn(page_folio(page)))) {
				low_pfn--;
				continue;
			}
		...
		/*
		 * Background compaction: skip folios that sit in the youngest
		 * MGLRU generation (max_seq), i.e. the most recently accessed
		 * pages, to avoid disturbing hot memory.  This is a lockless
		 * best-effort check; a racy generation read only affects which
		 * pages we skip, not correctness.
		 */
		if (!cc->direct_compaction && !cc->alloc_contig &&
		    lru_gen_enabled()) {
			...
			if (gen >= 0 &&
			    gen == lru_gen_from_seq(READ_ONCE(lruvec->lrugen.max_seq)))
				goto isolate_fail_put;
		}
