...
static void hermit_high_work_func(struct work_struct *work)
{
	...
	if (!work || (unsigned long)work < PAGE_SIZE) {
		pr_err("%s:%d\n", __func__, __LINE__);
		return;
	}

	hmt_ws = container_of(work, struct hmt_work_struct, work);
	if (!hmt_ws || (unsigned long)hmt_ws < PAGE_SIZE) {
		pr_err("%s:%d\n", __func__, __LINE__);
		return;
	}
	id = hmt_ws->id;
	if (id < 0 || id >= HMT_MAX_NR_STHDS) {
		return;
	}
	memcg = container_of(hmt_ws, struct mem_cgroup, sthds[id]);
	if(!memcg || (unsigned long)memcg < PAGE_SIZE) {
		pr_err("%s:%d\n", __func__, __LINE__);
		return;
	}
