		     ...
		     unsigned long address, void *arg)
{
	...
	static int thp_unmap_count = 0;
	
	/* [DEBUG] Track THP unmap entry */
	if (folio_test_large(folio) && folio_test_anon(folio)) {
		SWAP_DEBUG_LOG("try_to_unmap_one ENTER: folio=%px addr=%lx\n",
			       folio, address);
	}
	...
	while (page_vma_mapped_walk(&pvmw)) {
		...
		if (PageHWPoison(subpage) && !(flags & TTU_IGNORE_HWPOISON)) {
			...
			/* [DEBUG] Track THP PTE unmap progress */
			if (folio_test_large(folio)) {
				if ((++thp_unmap_count % 128) == 0) {
					SWAP_DEBUG_LOG("THP unmap progress: %d PTEs unmapped\n", thp_unmap_count);
				}
			}
			/*
