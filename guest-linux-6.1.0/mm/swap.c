...
/*
 * PFN from the host restore ring may refer to any page within a THP;
 * page_folio() yields the head where PG_blowfish_host_reported is stored.
 */
bool blowfish_restore_reported_folio(unsigned long pfn)
{
	...
	if (!folio_test_blowfish_host_reported(folio))
		return false;

	folio_clear_blowfish_host_reported(folio);
	folio_add_lru(folio);
	// folio_put(folio);
	return true;
}
EXPORT_SYMBOL_GPL(blowfish_restore_reported_folio);

static void check_blowfish_buffer(void)
{
	u64 pfn;

	while (blowfish_restore_buffer_pop(&pfn)) {
		if (pfn_valid(pfn))
			blowfish_restore_reported_folio(pfn);
		cond_resched();
	}
}
