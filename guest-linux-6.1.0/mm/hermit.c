 ...
 * utils
 */
void hmt_update_rft_dist(struct folio *folio, unsigned rft_dist)
{
	struct mem_cgroup *memcg = NULL;
	struct hmt_swap_ctrl *sc = NULL;
	if (!hmt_ctl_flag(HMT_APT_RECLAIM) || !folio)
		return;
...
int hermit_folio_referenced(struct vpage *vpage, struct folio *folio,
			   ...
			   unsigned long *vm_flags)
{
	...
	*vm_flags = 0;
	...
	if (vma->vm_flags & VM_LOCKED) {
		...
		if (pte && !folio_test_hugetlb(folio))
			pte_unmap(pte);
		*vm_flags |= VM_LOCKED;
		goto walk_done;
	}
