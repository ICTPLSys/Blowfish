...
/*
 * TODO: assign a finalized hypercall number with host side handler.
 * Return 0 to allow THP swap allocation path to continue;
 * non-zero asks reclaim to fall back to THP split path.
 */
...
bool add_to_swap_profiling(struct folio *folio, uint64_t pf_breakdown[])
{
	...
	if (folio_test_large(folio)) {
		hc_ret = kvm_hypercall2(KVM_HC_THP_SWAP_GATE,
					page_to_pfn(&folio->page),
					folio_nr_pages(folio));
		if (hc_ret) {
			SWAP_DEBUG_LOG("THP swap gate denied: folio=%px pfn=%lx nr_pages=%lu hc_ret=%ld\n",
				       folio, page_to_pfn(&folio->page),
				       folio_nr_pages(folio), hc_ret);
			return false;
		}
	}
	 ...
	 */
	/* [DEBUG] Track swap allocation success */
	if (folio_test_large(folio)) {
		SWAP_DEBUG_LOG("folio_alloc_swap SUCCESS: folio=%px entry=%lx\n",
			       folio, entry.val);
	}
	...
	if (err) {
		/* [DEBUG] Track cache add failure */
		if(folio_test_large(folio)) {
			SWAP_DEBUG_LOG("add_to_swap_cache FAILED: folio=%px err=%d\n",
				       folio, err);
		}
		/*
	...
	/* [DEBUG] Track cache add success */
	if (folio_test_large(folio)) {
		SWAP_DEBUG_LOG("add_to_swap_cache SUCCESS: folio=%px\n", folio);
	}
	/*
...
{
	return add_to_swap_profiling(folio, NULL);
}
...
struct page *__read_swap_cache_async_profiling(swp_entry_t entry, gfp_t gfp_mask,
			struct vm_area_struct *vma, unsigned long addr,
			bool *new_page_allocated, int *adc_pf_bits, uint64_t pf_breakdown[])
{
	struct swap_info_struct *si;
	struct folio *folio;
	void *shadow = NULL;
	...
	*new_page_allocated = false;
	...
	accum_adc_time_stat(ADC_ALLOC_PAGE_LAT, pf_ts);
#endif
	/*
	...
	if (hermit_mem_cgroup_swapin_charge_folio(folio, vma->vm_mm, gfp_mask, entry,
	...
	if (shadow) {
		unsigned long rft_dist =
			hermit_workingset_refault(folio, shadow);
		if (rft_dist) // [Hermit] update refault distance
			hmt_update_rft_dist(folio, rft_dist);
	}
...
{
	return __read_swap_cache_async_profiling(entry, gfp_mask, vma, addr,
			new_page_allocated, NULL, NULL);
}
...
{
	// struct swap_info_struct *si;
	struct folio *folio = NULL, *new_folio = NULL;
	void *shadow = NULL;
	...
	*new_page_allocated = false;
	...
	new_folio = vma_alloc_folio(gfp_mask, 0, vma, addr, false);
	if (!new_folio) {
		new_folio = filemap_get_folio(swap_address_space(entry),
				     swp_offset(entry));
		return &new_folio->page;
	}

	*cpu = hermit_issue_read(&new_folio->page, entry);
	// [RMGrid] profiling
	...
	/* May fail (-ENOMEM) if XArray node allocation failed. */
	if (add_to_swap_cache(folio, entry, gfp_mask & GFP_RECLAIM_MASK,
			      &shadow)) {
		goto fail_unlock;
	}
	// mem_cgroup_swapin_uncharge_swap(entry);
...
put_swap_folio(folio, entry);
	...
	if (cpu) {
		...
		hermit_poll_read(*cpu, &folio->page, true, pf_breakdown);
	}
	folio_put(folio);
	return NULL;
fail_free:
	if (cpu) {
		adc_profile_counter_inc(ADC_OPTIM_FAILED);
		hermit_poll_read(*cpu, &new_folio->page, true, pf_breakdown);
	}
	folio_put(new_folio);
	return &folio->page;
}
				   ...
				   struct swap_iocb **plug)
{
	...
	if (page_was_allocated) {
		...
		/*
		 * swap_readpage(..., true) may wait on disk I/O and schedule.
		 * Must not run under get_cpu(): preempt_disable breaks blk_io_schedule().
		 * (Frontswap/RDMA paths used to return without sleeping here.)
		 */
				...
				struct vm_fault *vmf)
{
	...
	fault_page = hmt_spec_io ?
		     __read_swap_cache_speculative(
				     ...
				     &demand_page_allocated, &cpu, NULL,
				     NULL) :
		     __read_swap_cache_async_profiling(
				     entry, gfp_mask, vma, addr,
				     &demand_page_allocated, NULL,
				     NULL);
	if (cpu == -1 && demand_page_allocated) {
		/* I/O may schedule; do not call under get_cpu()/preempt_disable. */
		swap_readpage(fault_page, do_poll, NULL);
		cpu = get_cpu();
		put_cpu();
		// [RMGrid] profiling
		// set_adc_pf_bits(adc_pf_bits, ADC_PF_MAJOR_BIT);
		adc_profile_counter_inc(ADC_ONDEMAND_SWAPIN);
		// fpath.realloc_page_fast(3);
		if (vma->vm_mm)
			count_memcg_event_mm(vma->vm_mm, ONDEMAND_SWAPIN);
	}
	...
	for (offset = start_offset; offset <= end_offset ; offset++) {
		...
		page = __read_swap_cache_async_profiling(
			...
			gfp_mask, vma, addr, &page_allocated, NULL, NULL);
		if (!page)
			continue;
		if (page_allocated) {
			swap_readpage_async(page, NULL);
...
skip:
	if (demand_page_allocated)
		...
		hermit_poll_read(cpu, fault_page, true, NULL);
...
// #endif
	...
	if (cpu == -1 && demand_page_allocated) {
		swap_readpage(fault_page, pref_req.ra_info.win == 1, NULL);
		cpu = get_cpu();
		put_cpu();
		// [RMGrid] profiling
		set_adc_pf_bits(adc_pf_bits, ADC_PF_MAJOR_BIT);
	...
	pf_ts = get_cycles_end() - pf_ts;
	if (demand_page_allocated)
		accum_adc_time_stat(ADC_PREFETCH_LAT, pf_ts);
#endif
	if (demand_page_allocated)
		// [Hermit] we disabled preemption inside poll_load when polling
