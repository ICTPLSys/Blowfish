...
if [[ $1 == "enable" ]]; then
  ...
  do
    ...
    sudo sh -c "echo 2800000 > /sys/devices/system/cpu/cpu$i/cpufreq/scaling_max_freq"
...
if [[ $1 == "disable" ]]; then
  ...
  do
    ...
    # 2. Must set both of the max and min freq
    sudo sh -c "echo 2800000 > /sys/devices/system/cpu/cpu$i/cpufreq/scaling_max_freq"
    sudo sh -c "echo 800000 > /sys/devices/system/cpu/cpu$i/cpufreq/scaling_min_freq"
