...
void fs_rdma_callback(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	ib_dma_unmap_page(ibdev, rdma_req->dma_addr, rdma_req->transfer_size,
...
int fs_build_rdma_wr(struct rdma_session_context *rdma_session,
		     ...
		     enum rdma_queue_type type, size_t transfer_size)
{
	int ret = 0;
	...
	rdma_req->page = page;
	rdma_req->transfer_size = transfer_size;
	dir = type == QP_STORE ? DMA_TO_DEVICE : DMA_FROM_DEVICE;
	rdma_req->dma_addr = ib_dma_map_page(dev, page, 0, transfer_size, dir);
	...
	ib_dma_sync_single_for_device(dev, rdma_req->dma_addr, transfer_size, dir);

	rdma_req->cqe.done = fs_rdma_callback;

	rdma_req->sge.addr = rdma_req->dma_addr;
	rdma_req->sge.length = transfer_size;
		    ...
		    enum rdma_queue_type type, size_t transfer_size)
{
	...
	chunk_num = rdma_session_global.remote_mem_pool.chunk_num;

	if (unlikely(chunk_idx >= chunk_num)) {
		pr_err("%s, invalid chunk_idx=%zu chunk_num=%zu offset=%lx page=%px size=%zu\n",
		       __func__, chunk_idx, chunk_num, (unsigned long)offset,
		       page, transfer_size);
		ret = -EINVAL;
		goto out;
	}
	...
	if (unlikely(remote_chunk_ptr->chunk_state != MAPPED)) {
		pr_err("%s, chunk_idx=%zu not mapped state=%d remote_addr=0x%llx rkey=0x%x offset_in_chunk=0x%zx size=0x%zx\n",
		       __func__, chunk_idx, remote_chunk_ptr->chunk_state,
		       remote_chunk_ptr->remote_addr, remote_chunk_ptr->remote_rkey,
		       offset_within_chunk, transfer_size);
		ret = -EINVAL;
		goto out;
	}

	if (unlikely(!remote_chunk_ptr->mapped_size)) {
		pr_err("%s, chunk_idx=%zu has zero mapped_size remote_addr=0x%llx rkey=0x%x offset_in_chunk=0x%zx size=0x%zx\n",
		       __func__, chunk_idx, remote_chunk_ptr->remote_addr,
		       remote_chunk_ptr->remote_rkey, offset_within_chunk,
		       transfer_size);
		ret = -EINVAL;
		goto out;
	}

	if (unlikely(offset_within_chunk + transfer_size >
		     remote_chunk_ptr->mapped_size)) {
		pr_err("%s, chunk bounds exceeded chunk_idx=%zu mapped_size=0x%llx offset_in_chunk=0x%zx size=0x%zx remote_addr=0x%llx rkey=0x%x\n",
		       __func__, chunk_idx, remote_chunk_ptr->mapped_size,
		       offset_within_chunk, transfer_size,
		       remote_chunk_ptr->remote_addr, remote_chunk_ptr->remote_rkey);
		ret = -EINVAL;
		goto out;
			  ...
			  struct page *page, size_t size)
{
	...
	ret = rswap_rdma_send(cpu, swap_entry_offset, page, QP_STORE, size);
	// if (size != PAGE_SIZE)
	// 	printk("[SWAP_DEBUG] rswap_frontswap_store: page=%px pfn=%lx offset=%lx size=%lu\n",
	// 			 page, page_to_pfn(page), swap_entry_offset, size);
	if (unlikely(ret)) {
...
int rswap_frontswap_store_on_core(unsigned type, pgoff_t swap_entry_offset,
				  struct page *page, int core, size_t size)
{
	int ret = 0;

	ret = rswap_rdma_send(core, swap_entry_offset, page, QP_STORE, size);
	if (unlikely(ret)) {
		pr_err("%s, enqueuing rdma frontswap write failed.\n",
		       __func__);
		goto out;
			 ...
			 struct page *page, size_t size)
{
	...
	cpu = get_cpu();
	// cpu = smp_processor_id();
	ret = rswap_rdma_send(cpu, swap_entry_offset, page, QP_LOAD_SYNC, size);
	// printk("[SWAP_DEBUG] rswap_frontswap_load: page=%px pfn=%lx offset=%lx size=%lu\n",
	// 			 page, page_to_pfn(page), swap_entry_offset, size);
	put_cpu();
	rswap_frontswap_poll_load(cpu);
	if (unlikely(ret)) {
		pr_err("%s, enqueuing rdma frontswap write failed.\n",
		       __func__);
		goto out;
			       ...
			       struct page *page, size_t size)
{
	...
	cpu = get_cpu();
	ret = rswap_rdma_send(cpu, swap_entry_offset, page, QP_LOAD_ASYNC, size);
	put_cpu();
	if (unlikely(ret)) {
		pr_err("%s, enqueuing rdma frontswap write failed.\n",
		       __func__);
